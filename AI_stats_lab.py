...
def evaluate_k_values(X, k_values, random_state=0, n_init=10):
    ...
    for k in k_values:
        ...
        objectives_list.append(result["objective"])

    for i in range(1, len(objectives_list)):
        improvement = (objectives_list[i - 1] - objectives_list[i]) / objectives_list[i - 1]
...
def choose_elbow_k(k_values, objectives):
    ...
    # Create a line from the first and last points
    ...
    for k, obj in zip(k_values[1:-1], objectives[1:-1]):
        # Calculate the perpendicular distance from the point (k, obj) to the line
        distance = abs((y2 - y1) * k - (x2 - x1) * obj + x2 * y1 - y2 * x1) / np.hypot(y2 - y1, x2 - x1)
...
def identify_outliers_by_distance(X, centroids, labels, top_n=5):
    ...
    top_outliers = distances[:top_n]

    return {
        "indices": [idx for idx, dist in top_outliers],
        "distances": [dist for idx, dist in top_outliers]
    }
...
def plot_unlabeled_data(X, feature_names=None, title="Unlabeled Data"):
    ...
    """
    fig, ax = plt.subplots()
    ax.scatter(X[:, 0], X[:, 1], color='blue', alpha=0.5)
    ax.set_title(title)
